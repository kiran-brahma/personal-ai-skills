...
- **`library/` is outside the budget.** Codex scans the plugin's `skills/` directory and `~/.agents/skills`, never `library/`, so on-demand skills cost nothing until `$decide-skills` loads one.
