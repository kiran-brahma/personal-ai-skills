...
reference anywhere in this skill means that file, not a Skill call. This whole package lives in the
on-demand `library/` tier, so none of these is installed as a slash command; reach them through this
router or `decide-skills`.
