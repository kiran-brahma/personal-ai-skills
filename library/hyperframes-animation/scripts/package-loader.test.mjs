...
// (b) This repository contains skills, not the HyperFrames source monorepo, so
// the loader intentionally exercises its global-install fallback here.
test("hyperframesPackageSpec: this library falls back to @latest", async () => {
  const prev = process.env[ENV];
  delete process.env[ENV];
  try {
    const { hyperframesPackageSpec } = await import("./package-loader.mjs");
    const spec = hyperframesPackageSpec("@hyperframes/producer");
    assert.equal(spec, "@hyperframes/producer@latest");
  } finally {
