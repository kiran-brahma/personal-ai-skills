...
test("captions.mjs is byte-identical across the included creation workflows", () => {
  const [first, ...rest] = ["product-launch-video", "faceless-explainer"].map(
    (skill) => ({
      skill,
...
test("every included build-frame.mjs copy stages the style axis it promises", () => {
  for (const skill of ["product-launch-video", "faceless-explainer"]) {
