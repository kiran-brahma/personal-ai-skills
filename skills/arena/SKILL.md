---
name: arena
description: "Spawn parallel candidates at one task, pick a base, graft the strongest parts of the losers into it. Use when a single attempt would lock in the wrong shape."
...
3. Pick the runners. Use `arena runners` from `the current harness's model configuration` when present. Otherwise default to one each on `a high-reasoning model`, `a second model family`, `a fast exploration model`, `a high-reasoning model from another family`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
