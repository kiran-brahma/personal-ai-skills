---
name: how
description: "How does X work: code walkthroughs, runtime flow, subsystem architecture, and placement questions ('where should this live'). Use why for motivation."
...
- `model`: your configured how-explorer model (default `a fast exploration model`)
- `readonly`: `true`
...
- `subagent_type`: `generalPurpose`
- `model`: your configured how-explainer model (default `a high-reasoning model`)
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list (defaults `a high-reasoning model`, `a second model family`, `a fast exploration model`, `a high-reasoning model from another family`), all in a single message.
