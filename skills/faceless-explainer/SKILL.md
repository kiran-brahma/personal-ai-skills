...
Use it only after the source writing is approved for adaptation. For Kiran's essays, do not invoke it
while Content Fence is still doing private thinking, essay development, reader reconstruction, or
reader-response work. For company posts, use the confirmed output of
`business-blog-post-generator` as source material and preserve its evidence boundaries.
...
`npx hyperframes init "videos/<project>" --non-interactive --example=blank --skill=faceless-explainer` — use the pinned CLI available to the video project; do not update the global skill set as a side effect.
